...
n_delete = n_live//2
threshold = 1
num_mcmc_steps_ratio = 1 # Number of MCMC steps relative to n_dims
psd_padding = 100 # Padding for PSD estimation to avoid trigger wrap-around
...
#SFM might remove?
# Save the final PSD to a CSV file
print("PSDs saved to psd_main.csv")
...
def build_conditional_prior(ra_mean, ra_std, dec_mean, dec_std):
    if analysis == "BBH":
        prior_C = CombinePrior([
            ...
            UniformPrior(s_z_min,s_z_max, parameter_names=["s2_z"]),
            SinePrior(parameter_names=["iota"]),
            # NOTE: Using a narrow d_L prior to ensure a high SNR injection for this analysis.
            UniformPrior(60.0,70.0, parameter_names=["d_L"]),
...
if validation == False:
    ...
    df_true_values.to_csv("truth_metadata_main.csv", index=False)
    injection_parameter_df.to_csv("injection_parameters_all_main.csv", index=False)
...
for i in range(iterations):
    ...
    if validation:
        data_path = f"GW170817.csv" if analysis == "BNS" else "GW150914.csv"
    else:
        data_path = f"{analysis}_waveform_output_{i}_main.csv"

    #SFM may remove entirely
    # psd_path = "psd_main.csv"

    # df_strain = pd.read_csv(data_path)
    # df_psd = pd.read_csv(psd_path)
    # frequencies = jnp.array(df_strain["Frequency (Hz)"].values)
    
    # detectors = ifos

    # for det in detectors:
    #     det.data.frequencies = frequencies
    #     det.data.fd = jnp.array(df_strain[f"{det.name} Strain"].apply(complex).values, dtype=jnp.complex128)
    #     det.psd.values = jnp.array(df_psd[f"{det.name} PSD"].values, dtype=jnp.float64)
    ...
    #     return likelihood_function(params, waveform_sky, detectors, frequencies, align_time)
    ...
    output_path_U = f"samples_{analysis}_unconditioned_{i}_main.csv"
    ...
    #SFM jnp.vstack([
    #     jax.random.uniform(init_keys[0], (n_live,), minval=M_c_min, maxval=M_c_max),
    #     jax.random.uniform(init_keys[1], (n_live,), minval=q_min, maxval=q_max),
    #     jax.random.uniform(init_keys[2], (n_live,), minval=s_z_min, maxval=s_z_max),
    #     jax.random.uniform(init_keys[3], (n_live,), minval=s_z_min, maxval=s_z_max),
    #     jax.random.uniform(init_keys[4], (n_live,), minval=lambda_min, maxval=lambda_max),
    #     jax.random.uniform(init_keys[5], (n_live,), minval=lambda_min, maxval=lambda_max),
    #     jnp.arcsin(jax.random.uniform(init_keys[6], (n_live,), minval=-1, maxval=1)), # Isotropic iota
    #     jax.random.beta(init_keys[7], 2.0, 2.0, (n_live,)) * (d_L_max - d_L_min) + d_L_min,
    #     jax.random.uniform(init_keys[8], (n_live,), minval=t_c_min, maxval=t_c_max),
    #     jax.random.uniform(init_keys[9], (n_live,), minval=phase_c_min, maxval=phase_c_max),
    #     jax.random.uniform(init_keys[10], (n_live,), minval=psi_min, maxval=psi_max),
    #     jax.random.uniform(init_keys[11], (n_live,), minval=ra_min, maxval=ra_max),
    #     jnp.arcsin(jax.random.uniform(init_keys[12], (n_live,), minval=-1, maxval=1)) # Isotropic dec
    # ]).T
    ...
    output_path_C = f"samples_{analysis}_conditioned_{i}_main.csv"
    ...
    #SFM initial_particles_C = jnp.vstack([
    #     jax.random.uniform(init_keys[0], (n_live,), minval=M_c_min, maxval=M_c_max),
    #     jax.random.uniform(init_keys[1], (n_live,), minval=q_min, maxval=q_max),
    #     jax.random.uniform(init_keys[2], (n_live,), minval=s_z_min, maxval=s_z_max),
    #     jax.random.uniform(init_keys[3], (n_live,), minval=s_z_min, maxval=s_z_max),
    #     jax.random.uniform(init_keys[4], (n_live,), minval=lambda_min, maxval=lambda_max),
    #     jax.random.uniform(init_keys[5], (n_live,), minval=lambda_min, maxval=lambda_max),
    #     jnp.arcsin(jax.random.uniform(init_keys[6], (n_live,), minval=-1, maxval=1)),
    #     jax.random.beta(init_keys[7], 2.0, 2.0, (n_live,)) * (d_L_max - d_L_min) + d_L_min,
    #     jax.random.uniform(init_keys[8], (n_live,), minval=t_c_min, maxval=t_c_max),
    #     jax.random.uniform(init_keys[9], (n_live,), minval=phase_c_min, maxval=phase_c_max),
    #     jax.random.uniform(init_keys[10], (n_live,), minval=psi_min, maxval=psi_max),
    #     jax.random.normal(init_keys[11], (n_live,)) * ra_std + ra_mean,
    #     jax.random.normal(init_keys[12], (n_live,)) * dec_std + dec_mean
    # ]).T
...
for i in range(iterations):
    samplesU_path = f"samples_{analysis}_unconditioned_{i}_main.csv"
    samplesC_path = f"samples_{analysis}_conditioned_{i}_main.csv"
...
for i in range(iterations):
    ...
    if not validation:
        sample_folder_C = f"samples_{analysis}_conditioned_{i}_main.csv"
        sample_folder_U = f"samples_{analysis}_unconditioned_{i}_main.csv"
    elif validation and analysis == "BNS":
        sample_folder_U = "samples_BNS_unconditioned_0_main.csv" # Assuming single run for validation
        sample_folder_C = "samples_BNS_conditioned_0_main.csv"
    else: # validation and BBH
        sample_folder_U = "samples_BBH_unconditioned_0_main.csv"
        sample_folder_C = "samples_BBH_conditioned_0_main.csv"
    ...
    if validation:
        filename = f"GW170817_data_main.csv" if analysis == "BNS" else f"GW150914_data_main.csv"
    else:
        filename = f"{analysis}_{i}_data_main.csv"
